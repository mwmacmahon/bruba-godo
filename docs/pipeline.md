...
**Context Isolation:** Uses `tools/helpers/convert-doc.py` for document analysis to prevent CC context bloat. The script makes isolated API calls — document content dies when the script exits, CC only sees the analysis results.
