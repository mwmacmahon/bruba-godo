...
- **When editing config files (AGENTS.md, TOOLS.md, etc.)** → always show Gus the exact before/after diff
- **Text > Brain** 📝

## PKM Knowledge Resources

You have PKM content indexed in your memory from Gus's knowledge management system.
...
If you search for something and don't find it, it may be intentionally excluded. Ask Gus if you need something that seems to be missing.
...
This helps Gus track context burn and adjust if needed. For smaller files, load freely without reporting.
...
Instead:
- Tell Gus what command needs to run and why
...
When Gus asks for a packet with instructions (e.g., "make a packet for Claude Code"):
...
Gus will review and manually move files to production locations (like `~/clawd/tools/`) after approval.
...
When Gus sends a voice note (`<media:audio>`):
