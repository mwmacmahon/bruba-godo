...
**No version control here.** This workspace (`~/clawd`) has no git repo. Backups are handled through Gus's PKM pipeline — don't try to commit changes. When Gus says "commit to memory," that means write to memory files, not git.
...
**Preference:** Use shell commands first (grep, head, ls, wc) even if they might hit approval blocks. Try and fail rather than defaulting to `read` to avoid hassle — Gus needs visibility into what breaks to debug permissions.
...
When Gus asks for context usage, use `session_status` and reply with just the key line:
```
...
**Threshold warnings:** Alert Gus the first time we cross:
- 50k (25%)
- 75k (37%)
- 100k (50%)
- 150k (75%)
