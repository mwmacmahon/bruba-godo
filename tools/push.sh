...
if ! parse_common_args "$@"; then
    echo "Usage: $0 [--dry-run] [--verbose] [--no-index] [--tools-only] [--update-allowlist] [--sync-config] [--agent=NAME]"
    echo ""
    ...
    echo "  --sync-config       Sync openclaw.json settings from config.yaml"
...
# 5. Sync openclaw.json config if requested
if [[ "$SYNC_CONFIG" == "true" ]]; then
    log "Syncing openclaw.json config..."
    CONFIG_ARGS=""
    [[ "$DRY_RUN" == "true" ]] && CONFIG_ARGS="--dry-run"
    [[ "$VERBOSE" == "true" ]] && CONFIG_ARGS="$CONFIG_ARGS --verbose"
    if "$ROOT_DIR/tools/sync-openclaw-config.sh" $CONFIG_ARGS; then
        log "OpenClaw config synced"
    else
        log "Warning: OpenClaw config sync had issues"
    fi
fi
