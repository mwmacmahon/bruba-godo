...
#   agents/{agent}/exports/Transcript Inventory.md
...
# Generate inventories for a specific agent
generate_agent_inventories() {
    local agent="$1"
    ...
    for f in "$agent_export_dir/summaries/Summary - "*.md; do [[ -f "$f" ]] && summary_count=$((summary_count + 1)); done

    if [[ $transcript_count -gt 0 || $summary_count -gt 0 ]]; then
        {
            ...
            generate_section "Summaries" "$agent_export_dir" "summaries" "Summary - "
            echo "---"
            echo "*Auto-generated by bruba-godo*"
        } > "$agent_export_dir/Transcript Inventory.md"
        echo "  $agent: Transcript Inventory: $transcript_count transcripts, $summary_count summaries"
    fi
    ...
    for f in "$agent_export_dir/cc_logs/Claude Code Log - "*.md; do [[ -f "$f" ]] && cclog_count=$((cclog_count + 1)); done

    if [[ $((doc_count + prompt_count + refdoc_count + cclog_count)) -gt 0 ]]; then
        {
            ...
            generate_section "Claude Code Logs" "$agent_export_dir" "cc_logs" "Claude Code Log - "
            echo "---"
            echo "*Auto-generated by bruba-godo*"
        } > "$agent_export_dir/Document Inventory.md"
        echo "  $agent: Document Inventory: $doc_count docs, $prompt_count prompts, $refdoc_count refdocs, $cclog_count cc_logs"
    fi
}
...
for agent in "${CP_AGENTS[@]}"; do
    ...
    generate_agent_inventories "$agent" "$AGENT_EXPORT_DIR"
done
